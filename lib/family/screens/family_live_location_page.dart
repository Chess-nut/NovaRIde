...
class FamilyLiveLocationPage extends StatefulWidget {
  const FamilyLiveLocationPage({super.key});

  @override
  State<FamilyLiveLocationPage> createState() => _FamilyLiveLocationPageState();
}

class _FamilyLiveLocationPageState extends State<FamilyLiveLocationPage> {
  ...
  Future<void> _animateToRider(RiderLocation location) async {
  ...
  Set<Marker> _markers(RiderLocation location) => {
        Marker(
          ...
          infoWindow: InfoWindow(
            title: _repository.connectedRider.name,
            snippet: location.status,
          ),
        ),
      };
  ...
  Widget build(BuildContext context) {
    ...
    final location = _location;
    final isEmergency = rider.status == FamilyRiderStatus.accidentDetected ||
        rider.status == FamilyRiderStatus.sosActive;
                ...
                children: [
                  ...
                  if (isEmergency)
                    Positioned(
                      ...
                      left: 32,
                      right: 32,
                      child: _EmergencyBanner(status: rider.status.label),
                    ),
                  Positioned(
                    right: 20,
                    bottom: 132,
                  ...
                  Positioned(
                    ...
                    left: 18,
                    right: 18,
                    child: _RiderInfoCard(rider: rider, location: location),
                  ),
                ],
              ),
            ),
      ...
      child: Row(
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '● ${rider.status.label}',
                  style: TextStyle(color: rider.status.color, fontSize: 12, fontWeight: FontWeight.w700),
                ),
                const SizedBox(height: 8),
                Text(
                  unavailable ? 'Location unavailable' : '${location!.speedKmh.toStringAsFixed(0)} km/h',
                  style: const TextStyle(color: NovaColors.primaryText, fontSize: 20, fontWeight: FontWeight.w800),
                ),
                const SizedBox(height: 4),
                Text(
                  unavailable ? 'Rider location unavailable' : location!.address,
          ...
          Container(
            ...
            height: 14,
            decoration: BoxDecoration(
              color: rider.internetConnected ? NovaColors.green : NovaColors.secondaryText,
...
class _EmergencyBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          color: NovaColors.red.withValues(alpha: 0.94),
        ...
        child: Row(
          children: [
            ...
            Text(status, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
          ],
        ),
      );
}
